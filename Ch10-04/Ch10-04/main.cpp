...
ostream& operator<<(ostream& os, const Reading& r) {
    return os << '(' << r.hour << ',' << r.temperature << '~' << 'C' << ')';
}
...
istream& operator>>(istream& is, Reading& r)
{
    ...
    is >> ch1 >> hour >> ch2 >> temperature >> ch3 >> ch4 >> ch5;
    if (!is) return is;
    if (ch1 != '(' || ch2 != ',' || ch3 != '~' || ch4 != 'C' || ch5 != ')') {
...
int main(int argc, const char * argv[]) {
    ...
    for (int i=0; i<temps.size(); ++i) {
        double to_fah = (temps[i].temperature * (9 / 5)) + 32;
        temps2.push_back(to_fah);
    }
